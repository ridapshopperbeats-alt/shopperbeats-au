...
export default function TopNavbar({ megaMenuData, initialWishlistCount = 0 }: TopNavbarProps) {
  ...
  const {
    ...
    fulfilledTimeStamp: wishlistFulfilledAt,
  } = useGetWishlistQuery(undefined, {
    refetchOnReconnect: true,
    refetchOnFocus: true,
  });
  const wishlistCount = wishlistData?.total_items ?? wishlistData?.items?.length;

  useEffect(() => {
    if (!wishlistFulfilledAt || Date.now() - wishlistFulfilledAt > 60_000) {
      refetchWishlist();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]);
  const [lastKnownWishlistCount, setLastKnownWishlistCount] = useState(
  ...
  const profileImageRaw = personalData?.response?.profile_image?.trim();
  const hasProfileImage = !!profileImageRaw;
            ...
            <div className={`header-link account hidden lg:block`}>
              {isAuthenticated ? (
                hasProfileImage ? (
                  ...
                  >
                    <CircleUserRound size={20} aria-label="account" />
          ...
          >
            <div className="new-mega-column">
              <div className="mobile-main-category">
                ...
                >
                  <h5 className="!text-[14px] font-bold">What&apos;s On Sale</h5>
                </Link>
              </div>
            </div>
            ...
            <div className="new-mega-column">
              <div className="mobile-main-category">
                <Link
                  ...
                  className="flex items-center gap-2 w-full"
                >
                  <h5 className="!text-[14px] font-bold">Clearance</h5>
                </Link>
              </div>
            </div>
